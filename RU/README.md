...
3. Что будет с `let`:
    ...
    2. В функции `h` переменную `a2` нельзя будет объявить заново

4. Что будет с `const`:

    1.  Из-за блочной типизации `A3` будет видна всей программе, а `F#` только функции в которой объявлена, из-за чего в функции `g` `A3` будет видна, а `F3` нет
